...
void waiting_time(int processes[], int number_of_processes, int bt[], int wt[]) {
    wt[0] = 0; // czas oczekiwania dla pierwszego procesu jest równy 0
...
// funkcja znajduj¹ca œredni czas oczekiwania oraz zakoñczenia procesu
void fcfs_algorithm(int processes[], int number_of_processes, int bt[]) {
    ...
    // obliczamy œredni czas oczekiwania i zakoñczonia procesu
    for (int  i=0; i<number_of_processes; i++){
...
int main()
{
    ...
    for (int i=0; i<number_of_processes; i++) {
        ...
        // losowa d³ugoœæ czasu wykonania (liczba z zakresu 1-N)
        int a = rand() % N + 1;
        burst_time[i] = a;
    }
